"""Demo variables and the per-course-week difficulty ramp.

Ported near-verbatim from bp-help's ``controls.py``. Defines the fixed pool of
demo variables (``numbers``/``strings``/``lists``/``dicts``) that generated
expressions are built from, and derives a "course week number" from the real
calendar date that ``app.py`` uses to look up this week's ``topic_probs``,
``leaf_prob``, ``min_steps``/``max_steps``/``max_expr_len``, and score goal.
"""
...
# steps_widget.steps._steps() dispatches against steps_widget.steps's own
# module globals(), not the caller's -- mirror the demo variables there so
# tracing a generated expression can resolve foo/bar/.../accounts/records the
# same way this module's own eval()/globals()[...] calls do (see
# expressions.py). Static for the life of the process: computed once here,
# not re-synced if these names were ever reassigned.
